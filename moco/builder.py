...
class MoCo(nn.Module):
    ...
    """
    def __init__(self, base_encoder, dim=128, K=65536, m=0.999, T=0.07, mlp=False, feat_dim=2048, normalize=True, num_classes=1000, bias=True):
        """
        ...
        # append to feat_after_avg layer
        self.linear = nn.Linear(feat_dim, num_classes, bias=bias)
        self.linear_k = nn.Linear(feat_dim, num_classes, bias=bias)
        ...
        # create feature memory module, save the past features: feat_after_avg_q
    ...
    def _register_hook(self):
        ...
        layer_q = self._find_layer(self.encoder_q) # AdaptiveAvgPool2d(output_size=(1, 1))
    ...
    def _train(self, im_q, im_k, labels):
        ...
        # compute logits;
        logits_q = self.linear(self.feat_after_avg_q) #[N, C]

        return features, target, logits_q

    def _inference(self, image, feat_q=False):
        q = self.encoder_q(image)
        q = nn.functional.normalize(q, dim=1)
        encoder_q_logits = self.linear(self.feat_after_avg_q)
        if feat_q:
            return encoder_q_logits, self.feat_after_avg_q
        else:
            return encoder_q_logits

    def forward(self, im_q, im_k=None, labels=None, feat_q=False):
        if self.training:
           return self._train(im_q, im_k, labels) 
        else:
           return self._inference(im_q, feat_q=feat_q)
